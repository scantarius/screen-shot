...
btn.addEventListener("click", () => {
    ...
    html2canvas(srcElement).then(canvas => {
        if(btn.id === "take") return document.body.appendChild(canvas);
...
DigitalRain.prototype.setColumn = function (column = {}) {
    ...
    if (index <= destination) return self.glyphs[gsap.utils.random(0, self.glyphs.length - 1, 1)]
    else return cacheChars[index]
    })
...
DigitalRain.prototype.init = function () {
    ...
    self.pause = () => gsap.ticker.remove(self.renderMatrix)
    self.play = () => gsap.ticker.add(self.renderMatrix)
}
...
) 
{
    ...
    else if (y > lastDestination) alpha = lower
...
DigitalRain.prototype.render = function () {
    ...
    if (row > column.destination) self.tracker[x] = self.setColumn(column)
    }
}
